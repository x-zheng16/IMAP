...
def init_config():
    ...
    _key_path = os.environ.get('MUJOCO_PY_131_MJKEY_PATH')
    if _key_path and not os.path.exists(_key_path):
        raise error.MujocoDependencyError('MUJOCO_PY_131_MJKEY_PATH path does not exist: {}'.format(_key_path))

    mjpro_path = os.environ.get('MUJOCO_PY_131_MJPRO_PATH')
    if mjpro_path and not os.path.exists(mjpro_path):
        raise error.MujocoDependencyError('MUJOCO_PY_131_MJPRO_PATH path does not exist: {}'.format(mjpro_path))
